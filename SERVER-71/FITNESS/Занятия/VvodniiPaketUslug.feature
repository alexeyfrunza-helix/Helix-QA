...
Сценарий: 01. Я создаю данные для проверки вводного пакета услуг.
	...
	И Я создаю клиента
	И Я создаю вводный пакет услуг.
...
//Создание персонального занятия.
	...
	Если элемент формы с именем 'ПолеОснованияОплаты_0' стал равен 'Нет доступных оснований' Тогда
		...
		И из выпадающего списка с именем 'ПолеОснованияОплаты_0' я выбираю точное значение "Пробный пакет: \"$$ВводныйПакет$$\", 2 000 ₽"
...
//Проверка списания персональной услуги.
	...
	И в дополнение формы с именем 'СтрокаПоиска' я ввожу текст "$$ВводныйПакет$$"
	...
	И таблица 'Список' содержит строки по шаблону:
		...
		| "1" | "1,000"      | 'Пакет услуг "$$ВводныйПакет$$" * от $$Сегодня$$ * ' | "Запись на занятие * от $$Сегодня$$ *" | "$$УслугаПерсональная$$" |
...
//Проверка списания групповой услуги.
	...
	И в таблице 'Список' я перехожу к строке:
		| "Клиент"           | "Номенклатура"                                    |
		| "$$Клиент$$" | "<b><colorstyle -46>$$ВводныйПакет$$</></>" |
	И в таблице 'Список' я выбираю текущую строку
	И я нажимаю на кнопку с именем 'Find'
	И таблица 'Список' содержит строки по шаблону:
		| "Количество" | "Основание"                                          | "Регистратор"                          | "Услуга, сегмент"     |
		| "1,000"      | 'Пакет услуг "$$ВводныйПакет$$" * от $$Сегодня$$ * ' | "Запись на занятие * от $$Сегодня$$ *" | "$$УслугаГрупповая$$" |
